...
uint8_t read6502(uint16_t address)
{
	...
	if (address < RAM_SIZE)
		return (RAM[address]); // RAM memory

	// if ((address >= 0xA000)&&(address < 0xC000)) return BIOS[(address - 0xA000)];
	if (address >= 0xA000 && address <= 0xBFFF)
		return basic[(address - 0xA000)]; // BASIC ROM
	if (address >= 0xE000)
		return kernal[(address - 0xE000)]; // KERNAL ROM
	...
	if ((address >= 0xD000) && (address < 0xD800))
		return (0); // VIC-II register, we don't emulate that

	if (address >= 55296 && address <= 56295)
		return colorRAM[address - 55296]; // Color RAM
}

void write6502(uint16_t address, uint8_t value)
{
	if (address < RAM_SIZE) // Writing into RAM
		RAM[address] = value;

	else if (address >= 55296 && address <= 56295) // Writing into Color RAM
		colorRAM[address - 55296] = value;

	else if (address == 53281) // Background Color
		backgroundColor = value;

	else if (address == 53280) // Border Color
		borderColor = value;
}
...
inline static uint16_t getvalue()
{
	if (useaccum)
		return ((uint16_t)a);
	else
...
inline static uint16_t getvalue16()
{
